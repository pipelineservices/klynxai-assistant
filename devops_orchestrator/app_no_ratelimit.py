...
async def devops_exception_handler(request: Request, exc: DevOpsException):
    """Handle DevOps exceptions with structured responses"""
...
def health():
    ...
    # Check Dragon API
    try:
        dragon_result = dragon_client.decision_status("health-check-test")
        health_status["dependencies"]["dragon_api"] = "healthy"
    except Exception as e:
        health_status["dependencies"]["dragon_api"] = f"unhealthy: {str(e)[:50]}"
    ...
    try:
        ...
        health_status["dependencies"]["incident_api"] = f"unhealthy: {str(e)[:50]}"
        health_status["status"] = "degraded"

    # Check file system for idempotency store
    try:
        from pathlib import Path
        idempotency_file = Path("data/idempotency.json")
        if idempotency_file.exists():
            health_status["dependencies"]["state_store"] = "healthy"
        else:
            health_status["dependencies"]["state_store"] = "initializing"
    except Exception as e:
        health_status["dependencies"]["state_store"] = f"unhealthy: {str(e)[:50]}"
        health_status["status"] = "degraded"
...
try:
    ...
    print("✓ Dragon Playground routes loaded")
except Exception as e:
    print(f"⚠ Dragon Playground not available: {e}")
